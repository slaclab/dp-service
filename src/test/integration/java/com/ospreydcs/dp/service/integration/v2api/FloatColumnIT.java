    ...
    public void testFloatColumnFullApiCoverage() throws Exception {
        ...
        // positive queryData() test case
        {
            ...
            for (DataBucket queryResultBucket : queryResultBuckets) {
                assertEquals(DataBucket.DataCase.FLOATCOLUMN, queryResultBucket.getDataCase());
                assertTrue(pvNames.contains(queryResultBucket.getFloatColumn().getName()));
                assertEquals(numSamples, queryResultBucket.getFloatColumn().getValuesCount());
            }
        }
        ...
        // check that expected subscribeData() response is received
        {
            ...
            assertTrue(responseBucket.hasFloatColumn());
            assertEquals(subscriptionColumn, responseBucket.getFloatColumn());
        }
        ...
        final List<DataBucket> responseDataBuckets = ingestionStreamServiceWrapper.verifySubscribeDataEventResponse(
                ...
                DataBucket.DataCase.FLOATCOLUMN);
        assertEquals(1, responseDataBuckets.size());
        assertEquals(subscriptionColumn, responseDataBuckets.get(0).getFloatColumn());
